...
public interface AccountRepositoryJpa extends JpaRepository<Account, Long> {
    ...
    @Query("""
            ...
            JOIN Person p ON a.person.id = p.id
            WHERE a.accountStatus = ru.smaginv.debtmanager.dm.entity.account.AccountStatus.ACTIVE AND
    ...
    @Query("""
            SELECT SUM(a.amount) FROM Account a
            JOIN Person p ON a.person.id = p.id
            WHERE a.accountStatus = ru.smaginv.debtmanager.dm.entity.account.AccountStatus.INACTIVE AND
            a.accountType = :accountType AND p.user.id = :userId
            """)
    ...
    @Query("""
            ...
            a.accountStatus = ru.smaginv.debtmanager.dm.entity.account.AccountStatus.INACTIVE AND
            a.person.id IN (SELECT p.id FROM Person p WHERE p.user.id = :userId)
            """)
